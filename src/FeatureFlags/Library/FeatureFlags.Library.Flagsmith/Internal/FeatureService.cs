...
internal class FeatureService(IServiceProvider provider, FlagsmithClient client, ILogger<FeatureService> logger)
    : IFeatureService, IJsonFeatureService
{
    ...
    public async Task<bool> IsEnabledAsync(string key, IFeatureContext context, bool defaultValue = false)
    {
        try
        {
            ...
            var flags = await client.GetIdentityFlags(config.Key, config.Traits);
            return await flags.IsFeatureEnabled(key);
        }
        catch (FlagsmithClientError e)
        {
            logger.LogWarning(e, "Flagsmith encountered an error for {Flag}. Returning default value", key);
        }
        catch (Exception e)
        {
            logger.LogError(e, "Error during IsEnabledAsync check for Flag {Flag}. Returning default value", key);
        }
    ...
    public async Task<T> GetConfiguration<T>(string key, IFeatureContext context, T defaultValue = default) where T : class
    {
        ...
        {
            logger.LogWarning(e, "Flagsmith encountered an error when getting Configuration for Flag {Flag}. Returning default value",
                key);
        }
        catch (Exception e)
        {
            logger.LogError(e, "Error during GetConfiguration check for Flag {Flag}. Returning default value", key);
        }
    ...
    {
        if (provider.GetService(typeof(IContextProvider)) is IContextProvider contextProvider)
        {
